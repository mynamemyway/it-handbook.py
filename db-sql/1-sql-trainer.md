...
> ⚠️ Корректировка стиля (2026): в оригинале курса использовалась функция `IF()`. По актуальному гайду и стандарту ANSI SQL для множественных условий предпочтительно использовать конструкцию `CASE WHEN ... END` — она переносима между СУБД и читаемее при вложенности.
...
**Пример-сравнение:**
```sql
-- Просто уникальный список (правильно использовать DISTINCT)
SELECT DISTINCT author FROM book;

-- Уникальный список + расчеты (обязателен GROUP BY)
