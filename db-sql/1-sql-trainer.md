...
#### 📝 Фильтрация групповому признаку

Если нужно отфильтровать строки по групповому признаку (например, по общему количеству у автора), но саму группу (`author`) выводить в итоговый результат **не нужно**, группировку убирают во вложенный запрос.

* **Внешний запрос:** Выводит только целевые колонки (`title`, расчет стоимости) и не содержит `GROUP BY`.
* **Внутренний запрос:** Занимается технической работой — группирует данные, считает суммы и возвращает список идентификаторов/имен для фильтра `IN`.
