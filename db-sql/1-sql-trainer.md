...
Формулировка «отбирает уникальные элементы столбца» — неточная и ведет к неверному пониманию работы ключевого слова.

`DISTINCT` отбирает **уникальные сочетания значений во всех указанных в `SELECT` столбцах**, а не в каком-то одном.
...
Смотрится не значение какого-то одного столбца (в каждом из них есть повторяющиеся значения), а сочетание значений во всех трех указанных в запросе столбцах. Строки `(10, 20, 30, 'a')`, `(10, 20, 30, 'd')` и `(10, 20, 30, 'e')` схлопнулись в одну, потому что по выбранным столбцам `one`, `two`, `three` они идентичны.

Если бы в запросе был указан столбец `four`, то независимо от других столбцов вывелись бы все 6 строк — в `four` все значения уникальны, и он сделал бы каждое сочетание остальных столбцов уникальным.
