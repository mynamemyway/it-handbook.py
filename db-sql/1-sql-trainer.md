...
| **Дата и время** | `DATE` <br> `TIME` <br> `TIMESTAMP` / `DATETIME` | Только дата (ГГГГ-ММ-ДД). <br> Только время (ЧЧ:ММ:СС). <br> Дата и время одновременно (+ учет часовых поясов). | День рождения, дата старта. <br> Время работы, будильник. <br> Время создания заказа/поста. |
