...
dir()                   # список имен объекта или список объектов доступных в локальной области
divmod()                # вернет частное и остаток от деления двух чисел
enumerate()             # вернет объект, который генерирует кортежи из двух элементов (индекса и элемента)
...
ascii()                 # возвращает строковое представление объекта, заменяет не-ASCII символы на экранированные последовательности
...
property()              # возвращает свойство атрибута (для использования геттеров/сеттеров)

# Отладка и прочее
breakpoint()            # вызывает отладчик (pdb)
credits                 # информация об авторских правах
