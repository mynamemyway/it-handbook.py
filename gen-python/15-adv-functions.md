...
Встроенная функция `map()` имеет сигнатуру `map(func, iterable, *iterables)`. В отличие от нашей версии из прошлого урока, встроенная функция `map()` может принимать сразу несколько последовательностей, переменное количество аргументов.
...
*Обрати внимание на тернарный оператор внутри лямбды и немедленный вызов — это полезные трюки для функционального стиля.*
...
💡 **Техническая разница для собеседования:**
* **Выражение (Expression)** — код, который процессор вычисляет и превращает в конкретное значение (например, `x > 0` или `x + 5`).
