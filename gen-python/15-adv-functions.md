...
- Вариант А: Списочное выражение быстрееnums = `[int(x)**2 for x in strings]`
- Вариант Б: `map()` медленнее из-за лямбдыnums = `map(lambda x: int(x)**2, strings)`

📝 **Совет по сложности**

* Нужна чистая конвертация встроенной функцией (`int`, `str`) ➡️ Используйте `map()`.
* Нужна сложная математика или фильтрация с условиями ➡️ Используйте списочное выражение.
...
**Первый аргумент filter() — это строго объект функции (или None)**.
Передавать туда математические или логические выражения нельзя.
Если нужно отфильтровать массив простым выражением без `lambda`, вместо `filter()` лучше использовать списочное включение с условием if:
