...
![Примеры](../assets/images/gen-py/15-4-1.png)
...
#### 📝 Механика работы замыканий в C-API Python
...
# Создаём две разные функции-замыкания
f_square = pow(2)
f_cube = pow(3)

# Вызываем инструмент
result = f_square(9)  # Выведет 81
```
...
Чтобы легко конструировать вложенные функции, запомните простое правило деления обязанностей:
   ...
   3. Чтобы внутренняя функция стала замыкающей — **она должна использовать внутри своего тела хотя бы одну переменную из внешней, а внешняя функция должна возвращать внутреннюю по имени (без скобок)**
...
#### 📝 Фильтрация условий внутри предикатов `all()` / `any()`

Использование генераторных выражений в качестве аргументов встроенных функций-валидаторов.

- Механика: Передача синтаксиса `(условие for элемент в последовательности)` внутрь `all()` или `any()` создает ленивый генератор предикатов.
- Пример синтаксиса:
  ```python
  if all(int(g) > 64 for g in grades):
  ```
- Особенность выполнения: Функции `all()` и `any()` вычисляют элементы генератора по одному (лениво). `all()` остановит итерацию на первом `False`, а `any()` — на первом `True`. Это избавляет от необходимости писать громоздкие вложенные циклы `for/break`.
