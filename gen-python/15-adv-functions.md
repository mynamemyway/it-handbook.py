...
Решение 3 (вложенная функция) - студента
```python
import math
...
Решение 4 (вложенная функция) - моё с разбором
```python
from math import sin


# Объявляем функцию-генератор
def generator_func(f):
    """
    Принимает мат. выражение из словаря по ключу
    Возвращает ОБЪЕКТ вложенной функции
    Которая выполняет математическое выражение
    """
    def apply_func(n):
        return f  # Возвращает результат мат. выражения для n

    return apply_func  # Возвращает объект влож. функции apply_func
...
# Присваиваем переменной объект вложенной функции с выбранным выражением в аргументе
func = generator_func(functions[f_key])

# Вызываем сгенерированную функцию с аргументом n
print(func(n))

# Если хотим без промежуточной переменной func, пишем двойной вызов
# print(generator_func(functions[f_key])(n))
```
