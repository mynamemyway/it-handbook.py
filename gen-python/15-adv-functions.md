...
- Механика стандартного `zip()`: Прекращает работу, как только закончится самый короткий список. Лишние элементы длинных списков игнорируются (баг-уязвимость).
- Механика `zip(..., strict=True)`: Если длины коллекций различаются, Python сразу выбрасывает ошибку `ValueError`.
