...
Все математические методы **описаны в самом классе `Decimal`**. Когда вы импортируете класс и создаете число (экземпляр класса), этот объект автоматически получает доступ ко всем его методам через точку. Никакие дополнительные функции из модуля импортировать не нужно.
* **Пример:** `res = d.ln() + d.sqrt()`

#### 2. Почему `Decimal` нужно импортировать, а `list` — нет?
* **Встроенные типы (`list`, `str`):** Используются постоянно, всегда загружены в память по умолчанию.
* **Стандартная библиотека (`Decimal`):** Специализированный инструмент для точных вычислений. Python хранит его на «складе». Команда `import` доставляет его в код.
...
#### 4. Полный список математических методов `Decimal`
- `.exp()` — Экспонента (e^x)
